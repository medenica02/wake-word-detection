...
Sa slike \ref{fig:lfcc_loss} može se primetiti da vrednost funkcije gubitka postepeno opada tokom epoha, od približno 0.10 do 0.04, što ukazuje na uspešno treniranje modela i stabilno prilagođavanje parametara mreže zadatom problemu klasifikacije.
...
Za klasifikaciju audio uzoraka korišćen je podrazumevani prag odlučivanja od 0.96, pri čemu su izlazi modela veći ili jednaki zadatom pragu klasifikovani kao pozitivna klasa, odnosno prisustvo ključne reči ``stop''.
...
Na slici \ref{fig:lfcc_roc} prikazana je ROC (\textit{Receiver Operating Characteristic}) kriva LFCC modela. Dobijena AUC vrednost iznosi približno 0.95, što ukazuje na veoma dobru sposobnost modela da razlikuje pozitivne i negativne primere nezavisno od izbora praga odlučivanja.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{images/lfcc_roc_curve.png}
	\caption{ROC kriva LFCC modela}
	\label{fig:lfcc_roc}
\end{figure}

Na slici \ref{fig:lfcc_pr} prikazana je Precision-Recall kriva LFCC modela. Može se primetiti da precision opada sa povećanjem recall vrednosti, što je očekivano kod neuravnoteženih skupova podataka gde pozitivna klasa ima znatno manji broj uzoraka u odnosu na negativnu klasu.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{images/lfcc_pr_curve.png}
